...
## Phase 0.5 — Two-client test harness (prerequisite for Task 3 and Task 6)
...
> **Sequencing note (7r):** This harness is a hard prerequisite for Task 3's concurrent-edit survival test and Task 6's two-client suggestion convergence test. It MUST land before those tasks begin. It is promoted out of Phase 5 and positioned here (before Task 3) so the dependency is explicit in build order. Do not begin Task 3 until Task 13's smoke test is green.
...
**Prerequisite (7r):** Task 13 (the two-client collab test harness above) MUST be complete before this task begins — the concurrent-edit survival test below imports `makePeer()` from `tests/unit/collab-harness.ts`.
...
- [ ] **GREEN:** implement both functions:
  - `serializeAnchor`: read `binding = ySyncPluginKey.getState(editor.state).binding`; if absent (non-collab editor) return `null` (caller falls back to integer positions). `absolutePositionToRelativePosition(from, binding.type, binding.mapping)` and same for `to`; `relativePositionToJSON` each; return `{start, end}`. **`assoc` convention (RESOLVED — 7r):** anchor `start` with `assoc: -1` (the relative position binds to the Yjs item to its right, so text inserted exactly at the comment-start boundary lands OUTSIDE the anchor — i.e. the anchor does not absorb a prefix insertion). Anchor `end` with `assoc: 0` (binds to the Yjs item to its left, so text inserted exactly at the comment-end boundary lands OUTSIDE the anchor — the anchor does not silently grow). This is the "closed-interval on both ends" convention: a comment over `[from, to)` stays over exactly those characters under all boundary insertions. The test in the concurrent-survival case MUST assert this boundary behaviour — type a char exactly at `from` and one exactly at `to`, confirm neither is absorbed into the comment range. This convention is locked; do not re-open it.
...
- [ ] **RED → GREEN (SURVIVAL — the bar #3 test):** with the editor still bound, capture an anchor for "world", then **simulate a concurrent remote edit using the Task 13 harness**: `const peer1 = makePeer('<p>hello world</p>'); const peer2 = makePeer(); peer1.syncTo(peer2);` — serialize the anchor from peer1, then on peer2 insert "BIG " at the start of the paragraph, `peer2.syncTo(peer1)`. Now peer1's text is `<p>BIG hello world</p>`. Re-`resolveAnchor` the SAME stored JSON on peer1 → assert the resolved range still exactly covers "world" (its absolute `from` shifted by +4). Also assert that a char inserted exactly at the comment's `start` position does NOT enter the anchor range (boundary `assoc:-1` check), and a char inserted exactly at the comment's `end` position does NOT enter the anchor range (boundary `assoc:0` check). This proves the closed-interval `assoc` convention and CRDT anchor survival together.
...
- [ ] **RED (unit, pure):** `permissionToRole(permission): DocPermRole`: `view`→`'viewer'`; `comment`→`'commenter'`; `edit`→`'editor'`; `suggest`→`'editor'` (suggest is an edit-via-tracked-changes — document the choice: treat as `'editor'` role so `getDocAccess` grants `canEdit`; the tracked-changes enforcement is UI-layer only). Watch fail.
- [ ] **RED (integration):** `resolveShareGrant(token, password)` → `null` for missing/expired/wrong-password; else `{ role: DocPermRole }`. It composes the existing `resolveShare` (already drops expired) + `verifySharePassword`, then maps the share row's `permission` field to a `DocPermRole` (`'view'→'viewer'`, `'comment'→'commenter'`, `'edit'|'suggest'→'editor'`). Assert an expired token → `null` (proves bar #6 at the resolution layer). Watch fail.
- [ ] **GREEN:** implement both, reusing `shares-repo` (do not duplicate the argon2/expiry logic — call it). The token travels on **share-scoped routes** under `/api/share/[token]/...` (so the capability is bound to the URL the visitor already holds), NOT on the owner-auth `/api/docs/[id]/...` routes. `resolveShareGrant` derives `role` from the share row's `permission` level: `view→'viewer'`, `comment→'commenter'`, `edit|suggest→'editor'`. Export `type ShareGrant = { role: DocPermRole }` from `src/lib/docs/share-grant.ts`. Deliverable: unit + integration green.

> **Type-shape bridge to Task 8 (7r, canonical §1c):** `resolveShareGrant` returns `{ role: DocPermRole }` or `null`. `DocPermRole = 'viewer' | 'commenter' | 'editor'` (imported from `@/lib/authz/doc-access` — A's module). When H's routes call `getDocAccess({ shareGrant: { role } }, docId)` in Task 8, this matches A's canonical signature exactly: `shareGrant?: { role: DocPermRole } | null`. A's `getDocAccess` maps `role` to capabilities via its internal `PERM_ALLOWS` table — H does NOT map role to capabilities and does NOT pass `{ share, capabilities }`. Freeze `{ role: DocPermRole }` as the `ShareGrant` type before Task 8 starts.
...
**Type alignment (7r, canonical §1c):** `getDocAccess`'s `shareGrant` parameter type is `{ role: DocPermRole } | null` (A's canonical shape). `ShareGrant` exported from `src/lib/docs/share-grant.ts` is defined as `{ role: DocPermRole }` — this is structurally identical to what A accepts. A's `getDocAccess` maps `role` to capabilities internally via `PERM_ALLOWS`; H passes `{ role }` directly and does NOT pre-map to capabilities. Coordinate with Group A to confirm A's `getDocAccess` parameter type before Task 8 begins.

- [ ] **VERIFY:** confirm `@/lib/authz/doc-access` is present and its `getDocAccess({user?, shareGrant?}, docId)` accepts the `ShareGrant` type from `src/lib/docs/share-grant.ts` directly (Group A must ship it before H Task 9 can proceed — see build order §3 in reconciliation). If A has not landed, coordinate: A is responsible for exposing `getDocAccess`/`authorizeDocRoute` before H Task 9 starts; do not stub a local copy.
...
- [ ] **RED (integration — bar #5 probe):** `tests/integration/doc-access.test.ts` proves H's *call-sites* behave correctly: `getDocAccess({ shareGrant: { role: 'viewer' } }, docId)` → only `canView`; `{ role: 'commenter' }` → `canView+canComment`; `{ role: 'editor' }` → +`canEdit`; `canManage` false for all share grants; no user + no grant → all false. These assertions validate the imported function's contract as H depends on it. Watch fail (A module absent or mismatched interface).
...
**Cross-document IDOR safety (7e):** `deleteComment` and `setResolved` in `comments-repo.ts` MUST filter on BOTH `id` and `docId` — i.e. `and(eq(comments.id, commentId), eq(comments.docId, docId))`. This prevents: authenticate on doc A, supply the id of a comment that actually belongs to doc B, and silently mutate it. The `/api/docs/[id]/comments/[commentId]` route MUST assert the resolved comment's `docId === [id]` (the route's own doc id) and return `404` if they differ. This is an IDOR boundary, not a permissions check — it applies even when the user has `canManage` on the target doc.
...
- [ ] **RED (cross-doc 404 — 7e):** in `tests/integration/comments-perms.test.ts`, create comment C1 on doc A and comment C2 on doc B; authenticate as the owner; PATCH/DELETE `/api/docs/[docAId]/comments/[commentC2Id]` (valid comment id, wrong doc) → `404`. Assert C2 is NOT modified. PATCH/DELETE `/api/docs/[docBId]/comments/[commentC1Id]` → `404`. Assert C1 is NOT modified. Watch fail.
- [ ] **GREEN:** implement the share-scoped route: resolve token → `resolveShareGrant` → `getDocAccess({shareGrant})` → gate GET on `canView`, POST on `canComment`. Reuse `createThread`/`addReply`/`listComments`. The owner route: swap the `doc.ownerId !== user.id` lines for `const access = await getDocAccess({user}, id); if (!access.canView) return 404; … if (!access.canComment) return 403` on POST. Anchors: the share route accepts `anchorStart`/`anchorEnd` JSON + integer fallback identically to the owner route. Fix `deleteComment` and `setResolved` in `comments-repo.ts` to filter `and(eq(id, commentId), eq(docId, docId))`. The `[commentId]` route handler: after resolving the comment row, if `comment.docId !== params.id` return `404` (IDOR guard). Deliverable: every perms case green; the view-link-cannot-comment bar is proven server-side with no cookie present; cross-doc 404 tests green.
...
## Phase 6 — Authenticated collab server handshake (REQUIRED)

### Task 15 (SHARE-EDIT): `onAuthenticate` gate on the collab server — server-gated handshake

**Files:** modify `collab/server.ts` (`onAuthenticate`), `Editor.tsx` (pass a session-scoped collab token to the provider), `src/app/api/collab-token/route.ts` (mints a short-lived collab token for authenticated editors and for `edit`-permission share visitors); test `tests/integration/share-edit-auth.test.ts`.

> **REQUIRED (7h — promoted from stretch/deferrable):** The reconciliation §7h makes `onAuthenticate` **binding for v0.2.0**: `collab/server.ts` MUST reject any Yjs/Hocuspocus connection that does not carry a valid session, PAT, or share-grant matching `documentName` with the needed capability. A view-only or expired grant MUST be rejected for edit connections. This is not optional — an unauthenticated collab port is a security defect. The previous "flag for the user at planning review" note is withdrawn; this task ships with H.

> **Collab port isolation (7h):** Bind the Hocuspocus server to `127.0.0.1` (loopback only), never `0.0.0.0`. The collab WebSocket is proxied via the Next.js server (or a reverse-proxy config in the compose setup); the collab port itself is NEVER published externally. Add a comment in `collab/server.ts` header: `// SECURITY: Bind to 127.0.0.1 only. Never expose this port to external traffic — all WS connections are proxied through the Next.js server or the compose reverse proxy. Publishing this port directly bypasses onAuthenticate.` Document the same in the compose / env-var block.

- [ ] **RED (integration — unauthenticated WS rejected):** a WebSocket connection to the collab server with NO token (simulated in the integration test by calling `onAuthenticate` directly with `token: undefined` and a known docId) → throws / rejects. Watch fail (no hook today → any connection accepted).
- [ ] **RED (integration — view-grant cannot edit):** a connection with a valid non-expired `view` share token for the correct doc → rejects (has `canView` but NOT `canEdit`). Watch fail.
- [ ] **RED (integration — expired token rejected):** a connection with an expired `edit` share token → rejects at the WS layer (bar #6 at the WS layer). Watch fail.
- [ ] **RED (integration):** a connection with a valid non-expired `edit` share token for the correct `documentName` → accepted. A connection with an `edit` token for a DIFFERENT doc id than `documentName` → rejects (cross-doc IDOR, same principle as 7e). Watch fail.
- [ ] **GREEN:** add `onAuthenticate` to `collab/server.ts`: parse `token` from the connection context; if it looks like a session/PAT, call `getUserByToken`/`requireUser` + verify the user has `canEdit` for `documentName` via `getDocAccess`; if it looks like a share token, call `resolveShareGrant(token)` and require that the resolved share's `docId === documentName` AND the grant's `role` maps to edit capability (`grant.role === 'editor'`) — i.e. pass `getDocAccess({ shareGrant: grant }, documentName)` and check `canEdit`; else throw. Bind the listener to `127.0.0.1`. The client (`Editor.tsx`) fetches a short-lived collab token from `/api/collab-token` (session-authenticated endpoint, mints a signed opaque token containing `userId` + `docId` + expiry) and passes it to `new HocuspocusProvider({ token })`. Share-edit visitors receive their token from the share route mint step. Deliverable: WS auth gates all connections; unauthenticated, expired, view-only, and cross-doc connections are rejected; owner editing works via the minted session token; collab port is bound to `127.0.0.1`; integration tests green.
...
3. **Authenticated collab server (Task 15) — RESOLVED (7h):** `onAuthenticate` is **REQUIRED** for v0.2.0. The collab server MUST reject unauthenticated / view-only / expired connections. The `edit`-share live co-editing increment ships with Task 15. This question is closed; do not re-open it.

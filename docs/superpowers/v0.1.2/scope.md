...
| F2b | Prose table header readable on the page sheet (F2 follow-on, user-reported) | TODO | ☐ | ☐ | `th { background: var(--border) }` #5f6368 + dark `--page-ink` text = 2.66:1. Fix = page-scoped `--page-surface-muted`/`--page-border` (mirror F2). Branched off F2-updated release |
